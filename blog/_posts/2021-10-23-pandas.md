...
Note: `Series` object has no attribute `pivot`, a `groupby` object after aggregation can be `series`. You'd better use [`reset_index()`](https://pandas.pydata.org/docs/reference/api/pandas.Series.reset_index.html) to treate the index as a column.
